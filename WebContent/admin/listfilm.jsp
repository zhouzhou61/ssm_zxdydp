				...
				<table cellpadding="0" cellspacing="0" width="100%" class="sTable">
					...
					<c:forEach items="${filmList}" var="film">
						<tr align="center">
							...
							<td>${film.catename}</td>
							<td>${film.price}</td>
							<td>${film.recommend}</td>
							<td>${film.thestart}</td>
							<td>${film.theend}</td>
							<td>${film.hits}</td>
							<td>${film.sellnum}</td>
							<td><a href="film/getFilmById.action?id=${film.filmid}">编辑</a>&nbsp;&nbsp;<a
								href="film/deleteFilm.action?id=${film.filmid}" onclick="{if(confirm('确定要删除吗?')){return true;}return false;}">删除</a></td>
						</tr>
					</c:forEach>
				</table>
